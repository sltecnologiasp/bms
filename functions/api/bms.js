export async function onRequest({ request, env }) {
  ...
  try {
    // ADMIN LOGIN - Mantém teu original
    ...
    if (action === 'user_bms') {
      if (!token ||!token.startsWith('dXNlcjo')) return json({ ok: false }, 401); // btoa('user:')
    ...
    if (action === 'add_bms' && request.method === 'POST') {
      ...
      try {
        ...
        // Cria registro na tabela bms se não existir
    ...
    // DADOS DA BMS ESPECÍFICA
    if (action === 'data') {
      if (!token ||!token.startsWith('dXNlcjo')) return json({ ok: false }, 401);
      const userId = parseInt(atob(token).split(':')[1]);
      const code = url.searchParams.get('code');
      
      // Verifica se BMS pertence ao user
    ...
    if (action === 'update' && request.method === 'POST') {
      ...
      if (result.changes === 0) {
        return json({ ok: false, error: 'BMS nao cadastrada: ' + code }, 404);
      }
      return json({ ok: true });
    }

    // ROTAS ADMIN ANTIGAS - mantém compatibilidade
    if (action === 'listar') {
    ...
    // LOGIN DIRETO POR CÓDIGO - mantém compatibilidade
